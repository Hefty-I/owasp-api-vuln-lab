...
public class DataSeeder {
    ...
    CommandLineRunner seed(AppUserRepository users, AccountRepository accounts, PasswordEncoder passwordEncoder) {
        return args -> {
            if (users.count() == 0) {
                // FIX(API2): Hash passwords using BCrypt instead of storing plain text
                AppUser u1 = users.save(AppUser.builder()
                    .username("alice")
                    .password(passwordEncoder.encode("alice123"))  // Hash password with BCrypt
                ...
                AppUser u2 = users.save(AppUser.builder()
                    .username("bob")
                    .password(passwordEncoder.encode("bob123"))  // Hash password with BCrypt
